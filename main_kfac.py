...
if __name__ == '__main__':
    ...
    tau_range = torch.logspace(-1, 0.5, 10)  # range to optimise swag over
    tau = 0.8  # value when training once
    ...
    pretrained_net_path = 'KFAC/' + which_data + '/pretrained/model_50'
    pretrained_opti_path = 'KFAC/' + which_data + '/pretrained/opti_50'

    # Load data
    Data = LoadDataSet(which_data)
    X, y, Xval, yval = Data.load_data_for_CV(noise = 0.2)
    ...
    if action == 'optimise':
        ...
        optimise_with_temp(alpha_range=tau_range, params=params, temp_range=temp_range,
                           load_net_path=pretrained_net_path, load_opti_path=pretrained_opti_path,
                           model_type='kfac', which_data=which_data,
        ...
        run_kfac(tau = tau, params = params, load_net_path = pretrained_net_path, load_opti_path = pretrained_opti_path,
                 ...
                 path_to_bma = path_to_bma, idx_model = 0, N = X_train.shape[0], temp = temp,
                 test_each_epoch = test_each_epoch)